...
for (i in 1:1)
{
	...
	for (j in 1:3)
	{
		...
		#write Bugs
		#save(Bugs,file=paste(getwd(),"/coda_nogen_species",SpeciesList[i],"_conmod",j,"_chain",k,".RData",sep=""))
		...
		for (k in 1:1)
		{
			...
			#set parameters to record
			Params <- c("gbar","gtemp","grain","sbar","sgnat","sgpad","sfpc","srden","selev","sbd","sclay","spaw","sveg","saut","rbar","vsig","msig","gdbar","gdres","gdsig")
			#Params <- c("predp");	
			...
			#fit model
			ModsGen[[i]][[j]][[k]] <- bugs(Bugs_Data,inits,Params,"mod_auto_gen.txt",n.chains=nc,n.thin=nt,n.iter=ni,n.burnin=nb,debug=F,bugs.directory="c:/Program Files (x86)/WinBUGS14/",working.directory=getwd())
